...
public class USLocalizer {
	...
	public USLocalizer(Odometer odo, UltrasonicSensor us,
		...
		this.nav = new Navigation(this.odo, odetect, ultra);
	...
	public void doLocalization() {
		...
		if (locType == LocalizationType.FALLING_EDGE) {
			...
			// switch direction and wait until it sees no wall, latch the angle
			// and compute gammaB
			...
			// Compute where to turn such that the robot faces the y direction
			// and turn to that direction.
			odo.getPosition(pos);
			...
			// switch direction and wait until it sees a wall and compute gammaA
			// using the newly recorded angle pos[2]
			robot.rotateClockwise();
			// acquire and filter the data
			while (getFilteredData() > 0) {
				if (getFilteredData() <= 29 && filterCount < filterMax) {
			...
			// keep rotating until the robot sees no wall, compute angleB using
			// the newly recorded angle pos[2]
			robot.rotateClockwise();
			// acquire and filter the data
			while (getFilteredData() > 0) {
				if (getFilteredData() > 30 && filterCount < filterMax) {
